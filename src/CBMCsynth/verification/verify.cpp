...
// prints solution
void verifyt::display_solution(const solutiont &solution)
{
  message.status()<<"SOLUTION:"<<messaget::eom;
  for(const auto & f: solution.functions)
  {
    message.status()<<expr2sygus(f.first)<<"  =  "<<expr2sygus(f.second)<<messaget::eom;
  }
}

// prints counterexample
void verifyt::display_cex(const counterexamplet &cex)
{
  message.status()<<"Counterexample:"<<messaget::eom;
  for(const auto & c: cex.assignment)
  {
    message.status()<<expr2sygus(c.first)<<"  =  "<<expr2sygus(c.second)<<messaget::eom;
  }
}
    ...
    decision_proceduret &solver)
  {
    ...
    switch(result)
    {
      case decision_proceduret::resultt::D_SATISFIABLE:
      {
        ...
        message.debug()<<"Got a counterexample "<<messaget::eom;
      ...
      default:
      {
        message.error()<<"ERROR in verification\n"<<messaget::eom;
        return verifyt::resultt::FAIL;
      } 
    }
  }
