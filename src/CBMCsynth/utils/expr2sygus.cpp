...
std::string convert_constant(const constant_exprt &expr)
{
      ...
      expr_type.id() == ID_bv)
  {
    ...
    result += "(_ bv" + integer2string(value) + " " + integer2string(width) + ")";
  }
  ...
  else if (expr_type.id() == ID_floatbv)
  {
    ...
    // if(use_FPA_theory)
    {
      ...
      result += "((_ to_fp " + integer2string(e) + " " + integer2string(f) + ")" + " #b" + integer2binary(v.pack(), v.spec.width()) + ")";
    }
  }
  ...
  else if (expr_type.id() == ID_rational)
  {
    ...
    {
      result += "(/ " + value.substr(0, pos) + ".0 " + value.substr(pos + 1) + ".0)";
    }
  }
        ...
        bswap_expr.type().id() == ID_unsignedbv)
    {
      ...
      else
      {
        ...
        for (std::size_t byte = 0; byte < bytes; byte++)
        {
          ...
          result += "((_ extract " + integer2string(byte * bits_per_byte + (bits_per_byte - 1)) + " " + integer2string(byte * bits_per_byte) + ") bswap_op)";
          result += ')';
        }
...
std::string convert_minus(const minus_exprt &expr)
{
  ...
  else if (expr.type().id() == ID_vector)
  {
    ...
    for (mp_integer i = 0; i != size; ++i)
    {
      ...
      forall_operands(it, expr) tmp.copy_to_operands(
          index_exprt(
              *it,
...
std::string convert_mult(const mult_exprt &expr)
{
  ...
  else if (expr.type().id() == ID_fixedbv)
  {
    ...
    result += "((_ extract " + integer2string(spec.width + fraction_bits - 1) + " " + integer2string(fraction_bits) + ") ";
...
std::string convert_type(const typet &type)
{
  ...
  else if (type.id() == ID_code)
  {
    ...
    result += "Bool";
           ...
           type.id() == ID_c_bool)
  {
    result += "(_ BitVec " + integer2string(to_bitvector_type(type).get_width()) + ")";
  }
  else if (type.id() == ID_c_enum)
  {
    // these have a subtype
    result += "(_ BitVec " + integer2string(to_bitvector_type(to_c_enum_type(type).underlying_type()).get_width()) + ")";
  }
  else if (type.id() == ID_floatbv)
  {
    ...
    // if(use_FPA_theory)
    result += "(_ FloatingPoint " + integer2string(floatbv_type.get_e()) + " " + integer2string(floatbv_type.get_f() + 1) + ")";
    // else
    //   result += "(_ BitVec "
...
std::string convert_typecast(const typecast_exprt &expr)
{
  ...
  else if (dest_type.id() == ID_floatbv)
  {
    ...
    else if (src_type.id() == ID_bv)
    {
      ...
      // if(use_FPA_theory)
      {
        result += "((_ to_fp " + integer2string(dest_floatbv_type.get_e()) + " " + integer2string(dest_floatbv_type.get_f() + 1) + ") ";
        result += convert_expr(src);
        result += ')';
      }
    }
...
std::string convert_floatbv_typecast(const floatbv_typecast_exprt &expr)
{
  ...
  if (dest_type.id() == ID_floatbv)
  {
    if (src_type.id() == ID_floatbv)
    {
      ...
      // if(use_FPA_theory)
      {
        result += "((_ to_fp " + integer2string(dst.get_e()) + " " + integer2string(dst.get_f() + 1) + ") ";
    ...
    else if (src_type.id() == ID_unsignedbv)
    {
      ...
      // if(use_FPA_theory)
      {
        result += "((_ to_fp_unsigned " + integer2string(dst.get_e()) + " " + integer2string(dst.get_f() + 1) + ") ";
        result += convert_rounding_mode_FPA(expr.op1());
        result += " ";
        result += convert_expr(src);
        result += ")";
